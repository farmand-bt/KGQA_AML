...
def format_answer(question, sparql_results):
    ...
    # For simple single-value results, skip the LLM
    if len(sparql_results) == 1 and len(sparql_results[0]) == 1:
        value = list(sparql_results[0].values())[0]
        clean = _clean_uri(value)
        return clean
